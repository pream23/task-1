...
export const uploadFile = async ({
  ...
  console.log("Storage client endpoint:", storage.client.config.endpoint);
  try {
...
const createQueries = (
  ...
  if (types.length > 0) queries.push(Query.equal("types", types));
...
export async function getTotalSpaceUsed() {
  try {
    ...
    files.documents.forEach((file) => {
      const fileType = file.types as FileType;
